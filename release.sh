...
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  MX Locale Arch Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
update_aur_package() {
    ...
    local tarball_url="https://github.com/MX-Linux/mx-locale/archive/refs/tags/${version}.tar.gz"

    # Update source in PKGBUILD
    sed -i "s|source=.*|source=(\"${tarball_url}\")|" PKGBUILD

    # Remove git from makedepends if present
    sed -i "s/'git' //;s/ 'git'//" PKGBUILD
